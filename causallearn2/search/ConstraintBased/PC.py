...
from causallearn2.utils.PCUtils.BackgroundKnowledgeOrientUtils import \
...
def pc(
    ...
    if mvpc:  # missing value PC
        ...
        return mvpc_alg(data=data, skeleton = skeleton, node_names=node_names, alpha=alpha, indep_test=indep_test, correction_name=correction_name, stable=stable,
                        uc_rule=uc_rule, uc_priority=uc_priority, background_knowledge=background_knowledge,
                        verbose=verbose,
                        show_progress=show_progress)
    else:
        return pc_alg(data=data,skeleton = skeleton,node_names=node_names, alpha=alpha, indep_test=indep_test, stable=stable, uc_rule=uc_rule,
...
def mvpc_alg(
    ...
    # print(cg_pre.G.graph)
    # print(cg_pre.sepset)
    ###### rewrite my code find self-masking node
    # if method in ["SM-MVPC", 'LCS-MS']:
    #     prt_m = get_self_masking_parent_missingness(cg_pre, prt_m)
    # print(prt_m)
    
    ## b) Correction of the extra edges
    cg_corr = skeleton_correction(data, alpha, correction_name, cg_pre, prt_m, stable)
    # print(cg_corr.G.graph)
    # print(cg_corr.sepset)
    
    # if method in ["SM-MVPC", 'LCS-MS']:
    #     prt_m = get_self_masking_parent_missingness(cg_corr, prt_m)
    # print(prt_m)
    ...
    if skeleton:
        return cg_corr
    # ##### rewrite my code ANM orient some edges
    # cg = anm_orient(data, alpha, indep_test, cg_corr, show_progress=show_progress, stable = stable, verbose=verbose)
...
def anm_orient(
    ...
    for x in range(no_of_var):
        ...
        for S in combinations(Neigh_x, Pa_num):
            if anm(x, S, indep_test, data, alpha): # satisfy anm model
                Pa[x] = list(S)
                break
            Pa_num = Pa_num -1
        if show_progress:
...
def anm(x, S, indep_test, data, alpha) -> bool:
    #1��linear non-Gaussian or nolinear
    #2��linear non-Gaussian:
    #   linear regression
    ...
    X = tdel_data[:, involve_vars[0]]
    Pa_X = tdel_data[:, involve_vars[1:]]
    if indep_test.method in ['fisherz', 'mv_fisherz', 'mc_fisherz']:
        linreg = LinearRegression().fit(Pa_X, X)
        residuals = X - linreg.predict(Pa_X)
        for i in range(Pa_X.shape[1]):
            pa_x = Pa_X[i]
            indep_flag = hsic_test(pa_x, residuals, alpha)
            if indep_flag != 1:  # ��������ֹͣ����
                return False
    elif indep_test.method == 'kci':
        X_predict = ANMNonlinear().anm.regressor.estimate(Pa_X, X)
        ...
        raise NotImplementedError(f"CITest method {indep_test.method} is not implemented.")
...
def get_self_masking_parent_missingness(cg_pre, prt_m) -> Dict[str, list]:
    ...
    for var_miss in vars_miss:
        if is_self_masking_node(cg_pre, prt_m, var_miss):
            ...
            prt_m['prt'][col] = np.array([vars_miss[col]])
