...
from causallearn2.utils.KCI.KCI import KCI_CInd, KCI_UInd
...
class KCI(CIT_Base):
    ...
    def __call__(self, X, Y, condition_set=None):
        ...
        if self.missing:
            var = Xs + Ys + condition_set
            test_wise_deletion_XYcond_rows_index = np.flatnonzero(~np.isnan(self.data[:, var]).any(axis=1))
            p = self.kci_ui.compute_pvalue(self.data[test_wise_deletion_XYcond_rows_index][:, Xs], self.data[test_wise_deletion_XYcond_rows_index][:, Ys])[0] if len(condition_set) == 0 else \
                self.kci_ci.compute_pvalue(self.data[test_wise_deletion_XYcond_rows_index][:, Xs], self.data[test_wise_deletion_XYcond_rows_index][:, Ys], self.data[test_wise_deletion_XYcond_rows_index][:, condition_set])[0]
        else:
